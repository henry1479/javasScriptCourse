...
const game = {
     ...
     */
    start() {
        clearInterval(game.timer); // очищает интервал 
        game.setGameStatus(GAME_STATUS_STARTED);
        game.displayPoints(); // выводит поле с очками
        ...
        snake.parts = snake.setStartPosition();//змейку в начальную позицию
    ...
    pause() {
     ...
     /**
     * Функция возобновляет игру после паузы
     */
    continue(){
        game.setGameStatus(GAME_STATUS_STARTED);
        clearInterval(game.timer);// чистит интервал
        game.timer = setInterval(game.move,500);//снова запускает игру на интервале 500
    },
    ...
    stop() {
        ...
        // вот выводится функция в функции
        displayMessage = game.displayMessageGameOver();
        displayMessage();
    ...
    /**
     * Функция выводит поле очкыов в начале игр
     */
     ...
     * Пример замыкания: внутренняя функция обращается 
     * к внешнему лексическому окружению за p  yfпеременной field.
     * 
     * 
     */
